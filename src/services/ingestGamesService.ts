...
export type IngestGamesPayload = z.infer<typeof ingestBodySchema>

export async function ingestGamesForWeek(body: unknown) {
  const { week: requestedWeek, season: requestedSeason } = ingestBodySchema.parse(body)
        ...
        transactionPromises.push(
          tx.game.upsert({
            ...
            update: {
              ...
              homeTeamScore: home.score ? Number.parseInt(home.score, 10) : null,
              awayTeamScore: away.score ? Number.parseInt(away.score, 10) : null,
              winnerTeamId,
            },
            create: {
              id: event.id,
